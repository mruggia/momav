...
ESC32::ESC32(byte _pinDShot, Uart* _telem) :
pinDShot(_pinDShot), telem(_telem), 
...
void ESC32::init(long _throt_freq, long _telem_freq, int _poles) {
  throt_freq = _throt_freq;
  telem_freq = _telem_freq;
  poles = _poles;
  
  telem->begin(115200);
  
  for(int i =0; i<2000; i++) {
    send_throt(0.0, false);
...
void ESC32::run() {
  time_now = micros();
  if(!time_last_throt) { time_last_throt = time_now; }
  if(!time_last_telem) { time_last_telem = time_now; }

  if (time_now - time_last_throt >= throt_freq) {

    if (time_now - time_last_telem >= telem_freq) {
      read_telem();
      send_throt(throt, true);
      time_last_telem += telem_freq;
    } else {
      send_throt(throt, false);
    }
    time_last_throt += throt_freq;
  }
...
void ESC32::send_throt(double _throt, bool _req_telem) {
  
  int throt_int = ((int)(_throt*2000))+48;    // get throttle as integer for dshot (range 48-2047)
  throt_int = constrain(throt_int, 48, 2047); // 
...
// read esc telemetry
void ESC32::read_telem() {

  //check if message size is correct
  if (telem->available() != 10) {
    while (telem->available() > 0) { telem->read(); }
    return;
  }
